<?if($arResult["STORES"]):?>
	...
	if($arStoresIDs){
		...
		while($arStore = $dbRes->GetNext()){
			...
			$arStoresByID[$arStore['ID']]["ADDRESS"] = $arStoresByID[$arStore['ID']]["TITLE"].((strlen($arStoresByID[$arStore['ID']]["TITLE"]) && strlen($arStoresByID[$arStore['ID']]["ADDRESS"])) ? ',<br /> ' : '').$arStoresByID[$arStore['ID']]["ADDRESS"];
