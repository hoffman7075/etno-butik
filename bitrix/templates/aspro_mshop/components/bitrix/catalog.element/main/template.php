					...
					<div class="offers_img wof">
						...
						<?if($arFirstPhoto["BIG"]["src"]){?>
							<a href="<?=$arFirstPhoto["BIG"]["src"]?>" class="fancy_offer" title="<?=$title;?>">
								<img id="<? echo $arItemIDs["ALL_ITEM_IDS"]['PICT']; ?>" src="<?=$arFirstPhoto['SMALL']['src']; ?>" alt="<?=$alt;?>" title="<?=$title;?>" itemprop="image">
							</a>
						<?}else{?>
								...
								<?foreach($arResult["PRICES"] as $key => $arPrice){?>
									<?if($arPrice["CAN_ACCESS"]){
										...
										<?if($arPrice["VALUE"] > $arPrice["DISCOUNT_VALUE"] && $arParams["SHOW_OLD_PRICE"]=="Y"){?>
											<div class="price" id="<? echo $arItemIDs["ALL_ITEM_IDS"]['PRICE']; ?>"<?=($bMinPrice ? ' itemprop="offers" itemscope itemtype="http://schema.org/Offer"' : '')?>>
												<?if($bMinPrice):?>
