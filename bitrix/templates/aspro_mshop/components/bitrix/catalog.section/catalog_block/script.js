...
window.JCCatalogSection.prototype.setBuyBlock = function(th, obj)
{
	...
	if((obj.CONFIG.OPTIONS.USE_PRODUCT_QUANTITY_DETAIL && obj.CONFIG.ACTION == "ADD") && obj.CAN_BUY){
		...
		// if(arBasketAspro["BASKET"] && (arBasketAspro["BASKET"].indexOf(obj.ID) !== -1 || arBasketAspro["BASKET"].indexOf(obj.ID * 1) !== -1)){
...
window.JCCatalogSection.prototype.setPrice = function(price, measure)
{
	...
	if (!!this.obPrice){
		...
		if (price.DISCOUNT_VALUE !== price.VALUE && this.showOldPrice)
		{
			...
			$(this.obPriceOld).find('span').show().html(BX.Currency.currencyFormat(price.VALUE, price.CURRENCY, true));
			if(this.showPercent){
