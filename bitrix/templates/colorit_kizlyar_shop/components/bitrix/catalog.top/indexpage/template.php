...
<?php
/**
 * Пишу здесь код, потому что больше некуда :(
 * - Быдлокодер, 26 января 2017г., 15:54
 */
/**
 * Гебеков! Удали неинформативные комментарии!
 * - Начальник, 16 июня 2017г., 15:04
 */
if( $arParams["HEAD_COUNTER"] == "Y" )
{
    $intProductCount = count($arResult["ITEMS"]);

    if( $arParams['FILTER_NAME'] ) {
        $filter = $GLOBALS[$arParams['FILTER_NAME']];

        $result = CIBlockElement::GetList(array(), $filter, array());
        $intProductCount = $result;
...
<div class="col-md-8">
    <a class="b-block-header" href="<?=$arParams["HEAD_TEXT_URL"]?>">
        <h2 class="b-block-header__text"><?=$arParams["HEAD_TEXT"]?></h2>
        <span class="b-block-header__count"><?=$intProductCount?></span>
    </a>
    <div class="b-goods-list">
        <? foreach($arResult["ITEMS"] as $arItem): ?>
        <div class="col-md-4 b-goods-list__item">
                    ...
                    <div>
                        ...
                        <div class="item_info <?=$arParams["TYPE_SKU"]?>">
                            <div class="item-title">
                                <a href="<?=$arItem["DETAIL_PAGE_URL"]?>"><span><?=$arItem["NAME"]?> <br />
                                <?=(!empty($arParams["HEAD_TEXT_ITEMS"])?" (".$arParams["HEAD_TEXT_ITEMS"] .")": "")?></span></a>
                            </div>
                            <!-- <?=$arQuantityData["HTML"];?> -->
                                    ...
                                    <div class="count_d_block">
                                        ...
                                        <script>
                                            $(document).ready(function(){
                                                if( $('.countdown').size() ){
                                                    var active_to = $('.active_to_<?=$arItem["ID"]?>').first().text(),
                                                    date_to = new Date(active_to.replace(/(\d+)\.(\d+)\.(\d+)/, '$3/$2/$1'));
                                                    console.info(date_to);
                            ...
                            <?} elseif(!$arItem['OFFERS']) { ?>
								<div class="view_sale_block"></div>
							<? } ?>
                            <div class="hover_block clearfix">
                                <?if($arItem["OFFERS"]){?>
