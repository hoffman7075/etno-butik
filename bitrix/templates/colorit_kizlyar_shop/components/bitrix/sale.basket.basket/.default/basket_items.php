						...
						foreach ($arResult["GRID"]["HEADERS"] as $id => $arHeader):
							...
							?>
								...
								<td class="item">
									...
									<? if($arItem["PRODUCT_ID"] != 910 && $arItem["MAX_ETCH_COUNT"] > 0): ?>
										<div>
											<div class="head-grav">Добавить гравировку</div>
											<div class="body-grav">
												<input type="text" style="width:80%;">
												<a href="javascript:void(0)" onClick="etch_save(<?=$arItem["PRODUCT_ID"]?>, 1, this.parentNode.children[0].value);">ОК</a>
											</div>
										</div>
									<? endif; ?>
								</td>
...
?>
		...
		<div class="bx_ordercart_order_pay_center">
			...
			{
				?>
				<a href="javascript:void(0)" onclick="updateBasket();" class="checkout refresh">123<?=GetMessage("SALE_REFRESH")?></a>
				<?
			}
			?>
			<a href="javascript:void(0)" onclick="checkOut();" class="checkout"><?=GetMessage("SALE_ORDER")?></a>
											...
											<script>
												$('.head-grav').click(function(){
													$(this).siblings('.body-grav').slideToggle();
												})
											</script>
<?
else:
