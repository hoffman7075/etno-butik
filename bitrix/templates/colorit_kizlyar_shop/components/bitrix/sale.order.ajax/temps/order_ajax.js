...
(function() {
	...
	BX.Sale.OrderAjaxComponent = {
		...
		showError: function(node, msg, border, redError = false)
		{
			...
			var node = this.mainErrorsNode.querySelector(".alert");
			setTimeout(function(){
				if( redError == false ) {
					node.style.color = "#e83333";
					node.style.background = "#ffeeee";
				} else {
					node.style.color = "#228D28";
					node.style.background = "rgb(241, 246, 250)";
				}
			}, 400);
		},
			...
			for (k in errors)
			{
				...
				switch (k.toUpperCase())
				{
					...
					case 'AUTH':
						...
						{
							redError = false;
							for(let i = 0; blockErrors[i]; i++) {
								if( blockErrors[i].substr(0, "Ошибка регистрации нового пользователя".length) == 'Ошибка регистрации нового пользователя' ) {
									blockErrors[i] = "Ваш E-Mail используется на сайте. Войдите в свой личный кабинет.";
								}
							}
							this.showError(this.mainErrorsNode, blockErrors, true, true);
							this.animateScrollTo(this.mainErrorsNode, 800, 20);
							scroll = false;
						}
						else
		...
		getDeliveryLocationInput: function(node)
		{
			...
			if (location && location[0] && location[0].output)
			{
				...
				labelHtml = '<label class="bx-soa-custom-label" for="soa-property-' + locationId + '">'
					...
					+ (currentProperty.DESCRIPTION.length ? ' <small>' + currentProperty.DESCRIPTION + '</small>' : '')
					+ '</label>';
		...
		getPropertyRowNode: function(property, propsItemsContainer, disabled)
		{
			...
			else
			{
				...
				if (propertyDesc.length && propertyType != 'STRING' && propertyType != 'NUMBER' && propertyType != 'DATE')
					textHtml += ' <small>' + propertyDesc + '</small>';
