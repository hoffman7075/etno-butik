	...
	<form action="<?=$APPLICATION->GetCurPage();?>" method="POST" name="ORDER_FORM" id="bx-soa-order-form" enctype="multipart/form-data">
		...
		<div id="bx-soa-order" class="row bx-<?=$arParams['TEMPLATE_THEME']?>" style="opacity: 0">
			...
			<div class="col-sm-9 bx-soa">
				...
				<div class="free-delivery-block">Бесплатная доставка от 15000р</div>
...
<?if($arResult["PRIM"] == 1) 
echo "<div style='    padding: 25px 29px 0px 79px;''><p style='color:#E47936; margin-bottom:0px;'>У вас в заказе есть холодное оружие!  Для того чтобы мы могли обработать ваш заказ, прикрепите разрешение.<p></div>";
else 
echo "<div style='    padding: 25px 29px 0px 79px;''><p style='color:#3c763d; margin-bottom:0px;'>У Вас в заказе отсутствует холодное оружие. Загружать разрешение не обязательно.<p></div>";

;?>
					
					<div class="bx-soa-section-content container-fluid" style="padding-top: 0px">
...
$(document).ready(function(){
	document.querySelector("input[name='licenses_order']").checked = true;
	document.querySelector("label[for='licenses_order']").style.display = "none";
})
</script>