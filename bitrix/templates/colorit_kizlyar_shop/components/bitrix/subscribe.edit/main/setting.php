	...
	<td class="left_blocks">
		...
		<div class="filter label_block">
			<?foreach($arResult["RUBRICS"] as $itemID => $itemValue):?>
				<input type="checkbox" name="RUB_ID[]"  checked id="RUB_ID_<?=$itemValue["ID"]?>" value="<?=$itemValue["ID"]?>"<?if($itemValue["CHECKED"]) echo " checked"?> />
...
<?php if( COption::GetOptionString("aspro.mshop", "SHOW_LICENCE", "N") == "Y" ): ?>
	<tr valign='top'>
		<td colspan='2'> <input type="checkbox" name="licenses_subscribe"> Я согласен на <a href="/include/licenses_detail.php" target="_blank">обработку персональных данных</a></td>
	</tr>
<?php endif ?>
