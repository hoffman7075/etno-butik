...
$productSlider = CMShop::getSliderForItemExt($arResult, $arParams['ADD_PICT_PROP'], 'Y' == $arParams['ADD_DETAIL_TO_SLIDER']);
...
if($productSlider){
	foreach($productSlider as $i => $arImage){
		$arFile = CFile::ResizeImageGet($arImage["ID"], array("width" => 800, "height" => 600), BX_RESIZE_IMAGE_PROPORTIONAL, true, array());
		$productSlider[$i] = array_merge(
			
			$arImage, array(
				"BIG" => array('src' =>$arFile['src']),
				"SMALL" => CFile::ResizeImageGet($arImage["ID"], array("width" => 340, "height" => 340), BX_RESIZE_IMAGE_PROPORTIONAL, true, array()),
				"THUMB" => CFile::ResizeImageGet($arImage["ID"], array("width" => 56, "height" => 56), BX_RESIZE_IMAGE_PROPORTIONAL, true, array()),
				"BIGS" => array('src' => CFile::GetPath($arImage["ID"])),
			)
		);
	}
}
...
if ($arResult['CATALOG'] && isset($arResult['OFFERS']) && !empty($arResult['OFFERS']))
{
	...
	foreach ($arResult['OFFERS'] as $keyOffer => $arOffer)
	{
		...
		$offerSlider = CMShop::getSliderForItemExt($arOffer, $arParams['OFFER_ADD_PICT_PROP'], true); // $arParams['ADD_DETAIL_TO_SLIDER'] == 'Y'
		...
		if($arOffer['MORE_PHOTO']){
			foreach($arOffer['MORE_PHOTO'] as $i => $arImage){
				if($arImage["ID"]){
					...
					$arOffer['MORE_PHOTO'][$i]["BIG"] = CFile::ResizeImageGet($arImage["ID"], array("width" => 800, "height" => 600), BX_RESIZE_IMAGE_PROPORTIONAL, true, array());
					$arOffer['MORE_PHOTO'][$i]["BIGS"]['src'] = CFile::GetPath($arImage["ID"]);
					...
					$arOffer['MORE_PHOTO'][$i]["THUMB"] = CFile::ResizeImageGet($arImage["ID"], array("width" => 56, "height" => 56), BX_RESIZE_IMAGE_PROPORTIONAL, true, array());
				}
			}
	...
	if('TYPE_1' == $arParams['TYPE_SKU'] && $arResult['OFFERS'] ){
		foreach ($arResult['OFFERS'] as $keyOffer => $arOffer)
		{
			...
			$arAddToBasketData = CMShop::GetAddToBasketArray($arOffer, $totalCount, $arParams["DEFAULT_COUNT"], $arParams["BASKET_URL"], false, $arItemIDs["ALL_ITEM_IDS"], 'big_btn', $arParams);
			...
			$arOneRow = array(
				...
				'AVAILIABLE' => CMShop::GetQuantityArray($arOffer['CATALOG_QUANTITY'], array(), "Y"),
			...
			if($arOneRow["PRICE"]["DISCOUNT_DIFF"]){
				if($arOneRow["PRICE"]["DISCOUNT_DIFF_PERCENT"])
					$percent=$arOneRow["PRICE"]["DISCOUNT_DIFF_PERCENT"];
				else
	...
	/*set min_price_id*/
	if('TYPE_1' != $arParams['TYPE_SKU'] && $arResult['OFFERS'] ){
		$arResult['MIN_PRICE'] = CMShop::getMinPriceFromOffersExt(
			$arResult['OFFERS'],
...
if (!empty($arResult['DISPLAY_PROPERTIES'])){
	foreach ($arResult['DISPLAY_PROPERTIES'] as $propKey => $arDispProp){
		...
		{
			if(is_array($arDispProp['VALUE']))
				$arResult['DISPLAY_PROPERTIES'][$propKey]['VALUE'] = implode(' / ', $arDispProp['VALUE']);
...
if (intVal($arParams["IBLOCK_STOCK_ID"])){
	...
	$arResult["STOCK"]=CMshopCache::CIBLockElement_GetList(array('CACHE' => array("TAG" => CMshopCache::GetIBlockCacheTag($arParams["IBLOCK_STOCK_ID"]))), array("IBLOCK_ID" => $arParams["IBLOCK_STOCK_ID"], "ACTIVE"=>"Y", "ACTIVE_DATE" => "Y", "PROPERTY_LINK" => $arResult["ID"]), false, false, $arSelect);
}

/*services*/
if( !empty($arResult["PROPERTIES"]["SERVICES"]["VALUE"]) ){
	$arSelect = array("ID", "IBLOCK_ID", "IBLOCK_SECTION_ID", "NAME", "PREVIEW_PICTURE", "PREVIEW_TEXT", "DETAIL_PAGE_URL");
	$arResult["SERVICES"]=CMshopCache::CIBLockElement_GetList(array('CACHE' => array("TAG" => CMshopCache::GetIBlockCacheTag($arResult["PROPERTIES"]["SERVICES"]["LINK_IBLOCK_ID"]))), array("IBLOCK_ID" => $arResult["PROPERTIES"]["SERVICES"]["LINK_IBLOCK_ID"], "ACTIVE"=>"Y", "ACTIVE_DATE" => "Y", "ID" => $arResult["PROPERTIES"]["SERVICES"]["VALUE"]), false, false, $arSelect);
}

/*brand item*/
$arBrand = array();
if(strlen($arResult["DISPLAY_PROPERTIES"]["BRAND"]["VALUE"]) && $arResult["PROPERTIES"]["BRAND"]["LINK_IBLOCK_ID"]){
	$arBrand = CMshopCache::CIBLockElement_GetList(array('CACHE' => array("MULTI" =>"N", "TAG" => CMshopCache::GetIBlockCacheTag($arResult["PROPERTIES"]["BRAND"]["LINK_IBLOCK_ID"]))), array("IBLOCK_ID" => $arResult["PROPERTIES"]["BRAND"]["LINK_IBLOCK_ID"], "ACTIVE"=>"Y", "ID" => $arResult["DISPLAY_PROPERTIES"]["BRAND"]["VALUE"]));
	if($arBrand){
...
$arResult["NOTE"] = ! empty($arResult["PROPERTIES"]["NOTE"]["VALUE"]) ? $arResult["PROPERTIES"]['NOTE']['VALUE'] : null;
$arResult["KOMPL"] = ! empty($arResult["PROPERTIES"]["PROP_KOMPL"]["VALUE"]) ? $arResult["PROPERTIES"]['PROP_KOMPL']['VALUE'] : null;