	...
	if (showImage && !!this.obPict)
	{
		...
		if('src' in this.currentBigImg){
			if (this.currentBigImg.src){
				...
				$(this.obPict).attr('data-zoom-image',this.currentBigImg.src);
				$(".color-zoom").zoom({url: this.currentBigImg.src});
				//alert(this.currentBigImg.src);
	...
	if(countPhoto>1){
		...
		container.find('.thumbs').delegate('li:not(.current)', 'click', function(e){
			...
			if(srcBig){
				...
				//pict.parent().attr('data-zoom-image',srcBig);
				//pict.parent().attr('src',srcBig);
				pict.parent().attr('title',slider[index].TITLE);
				pict.parent().attr('alt',slider[index].ALT);
				pict.attr('title',slider[index].TITLE);
				pict.attr('alt',slider[index].ALT);
				pict.attr('data-zoom-image',srcBig);
				$(".zoomImg").remove();
				$(".color-zoom").zoom({url: srcBig});
				//pict.parent().attr('src',srcBig);
			}
		});
