...
<? $this->addExternalJS($templateFolder.'/jquery.elevatezoom.min.js', true) ?>
		...
		<div class="item_slider">
			...
			<div class="slides b-goods__image">
				<?if($showCustomOffer && !empty($arResult['OFFERS_PROP'])){?>
					<div class="offers_img wof photo-item"  >
						<?
						...
						<?if($arFirstPhoto["BIG"]["src"]){?>
							...
							<a class="color-zoom" href="javascript:void(0)" title="<?=$title;?>">
								<img id="<? echo $arItemIDs["ALL_ITEM_IDS"]['PICT']; ?>" src="<?=$arFirstPhoto['BIG']; ?>" data-zoom-image="<?=$arFirstPhoto['BIG']['src']; ?>" alt="<?=$alt;?>" title="<?=$title;?>">

							</a>
						<?}else{?>
							<a class="color-zoom" href="javascript:void(0)" title="<?=$title;?>" data-zoom-image="<?=$arFirstPhoto['SRC']; ?>">
								<img id="<? echo $arItemIDs["ALL_ITEM_IDS"]['PICT']; ?>" src="<?=$arResult["PREVIEW_PICTURE"]['SRC']; ?>" alt="<?=$alt;?>" title="<?=$title;?>">
							</a>
						<?}?>
					</div>
				<?}else{
					if($arResult["MORE_PHOTO"]){?>
						<div>
							<?foreach($arResult["MORE_PHOTO"] as $i => $arImage){?>
								...
								?>
								<div id="photo-<?=$i?>" <?=(!$i ? 'class="photo-item current"' : 'class="photo-item" style="display: none;"')?>>
									<?if(!$isEmpty){?>
										<img id='photo-<?=$i?>-thumb' src="<?=$arImage["SMALL"]["src"]?>" alt="<?=$alt;?>" title="<?=$title;?>" data-zoom-image="<?=$arImage["BIG"]["src"]?>"  alt="<?=$arImage["ALT"];?>" />
									<?}else{?>
			...
			<?php if (count($arResult['MORE_PHOTO']) < 2): ?>
				<script>

				var url = document.querySelector("#photo-0-thumb").dataset.zoomImage;

				$("#photo-0").zoom({url: url, callback: () => {
					$("#photo-0").mouseout();
					$("#photo-0").mouseover();
				}});
				</script>
					...
					<div class="wrapp_thumbs">
						<div class="thumbs" style="max-width:<?=ceil(((count($arResult['MORE_PHOTO']) <= 4 ? count($arResult['MORE_PHOTO']) : 4) * 70) - 10)?>px;">
							<ul class="slides_block" id="thumbs">
								<?foreach($arResult["MORE_PHOTO"]as $i => $arImage):?>
									<li class="photo-slider-item <?=(!$i ? ' current' : '')?>" data-id="<?=$i?>" data-zoom-image="<?=$arImage['BIG']['src']?>">
										<img src="<?=$arImage["THUMB"]["src"]?>" alt="<?=$arImage["ALT"];?>" title="<?=$arImage["TITLE"];?>" />
									</li>
								<?endforeach;?>
							</ul>
						</div>
			...
			<!--	<div class="top_info">
					...
					<?elseif($arResult["OFFERS"] && $arParams['TYPE_SKU'] == 'TYPE_1'):?>
						<div class="offer_buy_block buys_wrapp" style="display:none;">
							<div class="counter_wrapp b-counter"></div>
						</div>
					<?elseif($arResult["OFFERS"] && $arParams['TYPE_SKU'] != 'TYPE_1'):?>
              ...
              <?if($arParams["DISPLAY_WISH_BUTTONS"] != "N"):?>
                <?if(!$arResult["OFFERS"]):?>
                  <div class="wish_item text" data-item="<?=$arResult["ID"]?>" data-iblock="<?=$arResult["IBLOCK_ID"]?>">
                    <span class="value pseudo" title="<?=GetMessage('CT_BCE_CATALOG_IZB')?>" ></span>
                    <span class="value pseudo added" title="<?=GetMessage('CT_BCE_CATALOG_IZB_ADDED')?>"></span>
                  </div>
                <?else:?>
                  <div class="wish_item text " data-item="<?=$arResult["ID"]?>" data-iblock="<?=$arResult["IBLOCK_ID"]?>" data-props="<?=$arOfferProps?>">
                    <span class="value pseudo <?=$arParams["TYPE_SKU"];?>" title="<?=GetMessage('CT_BCE_CATALOG_IZB')?>"></span>
                    <span class="value pseudo added <?=$arParams["TYPE_SKU"];?>" title="<?=GetMessage('CT_BCE_CATALOG_IZB_ADDED')?>"></span>
                  </div>
                <?endif;?>
              <?endif;?>
...
<div class="tabs_section">
	<ul class="tabs1 main_tabs1 tabs-head">
		...
		<?if(strlen($arResult["DISPLAY_PROPERTIES"]["VIDEO"]["VALUE"]) || isset($arResult["DISPLAY_PROPERTIES"]["VIDEO_YOUTUBE"]["VALUE"]) || $arResult["SECTION_FULL"]["UF_VIDEO"] || $arResult["SECTION_FULL"]["UF_VIDEO_YOUTUBE"]):?>
			<li class="<?=(!($iTab++) ? ' current' : '')?>">
	...
	<ul class="tabs_content tabs-body">
		...
		<?if(strlen($arResult["DISPLAY_PROPERTIES"]["VIDEO"]["VALUE"]) || isset($arResult["DISPLAY_PROPERTIES"]["VIDEO_YOUTUBE"]["VALUE"]) || $arResult["SECTION_FULL"]["UF_VIDEO"] || $arResult["SECTION_FULL"]["UF_VIDEO_YOUTUBE"]):?>
			<li class="<?=(!($iTab++) ? ' current' : '')?>">
				<div class="video_block">
					...
					<?elseif (isset($arResult["DISPLAY_PROPERTIES"]["VIDEO_YOUTUBE"])):?>
	
					<?foreach ($arResult["DISPLAY_PROPERTIES"]["VIDEO_YOUTUBE"]["~VALUE"] as $arVideo):?>
						<?=str_replace("frameborder=\"0\"", "", $arVideo);?>
					<?endforeach;?>
		...
		<?if($arParams["SHOW_ADDITIONAL_TAB"] == "Y"):?>
			<li class="<?=(!($iTab++) ? ' current' : '')?>">
				<?$APPLICATION->IncludeFile(SITE_DIR."include/additional_products_description.php", array(), array("MODE" => "html", "NAME" => GetMessage('CIT_BCE_CATALOG_ADDITIONAL_DESCRIPTION')));?>
			</li>
		<?endif;?>
	</ul>
</div>
