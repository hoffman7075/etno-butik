...
<div class="style-switcher <?=($_COOKIE['styleSwitcher']=='open' ? 'active' : '')?>">
	<div class="header"><?=GetMessage("THEME_MODIFY")?><span class="switch"><img alt="theme" title="theme" src="<?=$this->GetFolder()?>/images/cogs.png"></span></div>
							...
							<?foreach($option["VALUES"] as $key => $value):?>
								...
								<?if($option["ID"] == 'COLOR_THEME' && $value["VALUE"] == 'CUSTOM'):?>
									<?if(isset($arResult["ITEMS"]['CUSTOM_COLOR_THEME'])):?>
										...
										?>
										<a class="color custom_color_theme <?=($bCurrent ? 'active' : '')?>" rel="nofollow"  data-name="<?=$value["VALUE"]?>" data-option-id="<?=strToLower($option["ID"])?>" data-option-value="<?=strToLower($value["VALUE"])?>">
									...
									<?continue;?>
								<?endif;?>
								<a class="<?=($option["TYPE"] == "COLOR" ? 'color' : '')?> <?=($bCurrent ? 'active' : '')?>" rel="nofollow" href="javascript:;" data-name="<?=$value["VALUE"]?>" data-option-id="<?=strToLower($option["ID"])?>" data-option-value="<?=strToLower($value["VALUE"])?>" data-color="<?=$value["COMPONENT_VALUE"]?>">
