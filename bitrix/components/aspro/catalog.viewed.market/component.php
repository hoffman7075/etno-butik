...
if(!CModule::IncludeModule('aspro.mshop')){
	$arResult['ERROR'] = GetMessage('MSHOP_T_ERROR_MODULE_NOT_INCLUDED_NOTE');
}
else{
	global $TEMPLATE_OPTIONS, $STARTTIME;

	$localKey = 'MARKET_VIEWED_ITEMS_'.SITE_ID;
	...
	if($arViewed && is_array($arViewed)){
		...
		foreach($arViewed as $PRODUCT_ID => $arItem){
			...
			if($PICTURE_ID = isset($arItem['1']) ? $arItem['1'] : false){
				$arImage = CFile::ResizeImageGet($PICTURE_ID, array('width' => 170, 'height' => 170), BX_RESIZE_IMAGE_PROPORTIONAL, true);
			}
		...
		$arResult['LAST_ACTIVE_FROM'] = $arResult['ITEMS'][count($arResult['ITEMS']) - 1]['ACTIVE_FROM'];
	}
	// time of last viewed product
